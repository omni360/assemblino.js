...
window.Physijs = (function () {
    ...
    Eventable.prototype.removeEvent = function (event_name) {
        if (!this._eventListeners) return;
        delete this._eventListeners[event_name];
    };
    Eventable.prototype.removeAllEvents = function () {
        if (!this._eventListeners) return;
        var _this = this;
        _.map(this._eventListeners, function (v, k) {
            delete _this._eventListeners[k];
        });
    };
    Eventable.prototype.dispatchEvent = function (event_name) {
        ...
        if (this._eventListeners.hasOwnProperty(event_name)) {
            var listeners = this._eventListeners[event_name];
    ...
    Eventable.make = function (obj) {
        _.extend(obj.prototype, _.pick(Eventable.prototype, [
            'addEventListener',
            'removeEventListener',
            'dispatchEvent',
            'removeEvent',
    ...
    convertWorldPositionToObject = function (position, object) {
        ...
        //_temp_matrix4_1.getInverse(object.matrixWorld);
        var pos = position.clone();
        //pos.applyMatrix4(_temp_matrix4_1);
        object.worldToLocal(pos);
        return pos;
    };

    Physijs.convertWorldPositionToObject = convertWorldPositionToObject;
    ...
    Physijs.PointConstraint = function (objecta, objectb, position) {
        ...
        this.positiona = convertWorldPositionToObject(position, objecta);

        if (objectb) {
            this.objectb = objectb._physijs.id;
            this.positionb = convertWorldPositionToObject(position, objectb);
        }
    };

    Physijs.HingeConstraint = function (objecta, objectb, position, axis, axisb) {
        Eventable.call(this);
    ...
    Eventable.make(Physijs.HingeConstraint);
    Eventable.make(Physijs.SliderConstraint);
    Eventable.make(Physijs.DOFConstraint);

    Physijs.Constraint = function (options) {
        Eventable.call(this);
        var defaults = {
            type: undefined, //'hinge', 'dof', 'slider'
            id: getObjectId(),
            objecta: undefined,
            objectb: undefined,
            positiona: undefined,
        ...
        if (!options.type || !options.objecta) {
            console.log(arguments);
            throw ("Physijs.Constraint arguments are insufficient:");
        }
        function positionWorldToLocal(object, point) {
            if (!object || !point) return undefined;
            object.worldToLocal(point);
            return point;
        }

        function axisWorldToLocal(object, axis) {
            if (!object || !axis) return undefined;
            axis.add(object.position);
            object.worldToLocal(axis);
            return axis;
        }

        if (!options.positionaIsLocal) {
            options.positiona = positionWorldToLocal(options.objecta, options.positiona);
        }
        if (!options.positionbIsLocal) {
            options.positionb = positionWorldToLocal(options.objectb, options.positionb);
        }
        if (!options.axisaIsLocal) {
            options.axisa = axisWorldToLocal(options.objecta, options.axisa);
            options.fronta = axisWorldToLocal(options.objecta, options.fronta);
        }
        if (!options.axisbIsLocal) {
            options.axisb = axisWorldToLocal(options.objectb, options.axisb);
            options.frontb = axisWorldToLocal(options.objectb, options.frontb);
        }
        options.position = options.positiona;
        options.axis = options.axisa;
        options.front = options.fronta;
        options.rotationa = _.pick(options.objecta.rotation, ['x', 'y', 'z']);
        if (options.objectb) {
            options.rotationb = _.pick(options.objectb.rotation, ['x', 'y', 'z']);
        }
        _.extend(this, options);
        this.scene = sceneOf(Connector.prototype.simulator);
        this.appliedImpulse = 0;
        if (this.objecta) this.objecta = this.objecta._physijs.id;
        if (this.objectb) this.objectb = this.objectb._physijs.id;
        var methods = {
            hinge: ['setLimits', 'enableAngularMotor', 'disableMotor'],
            slider: ['setLimits', 'setRestitution', 'enableLinearMotor', 'disableLinearMotor', 'enableAngularMotor', 'configureAngularMotor', 'disableAngularMotor'],
            dof: ['setLinearLowerLimit', 'setLinearUpperLimit', 'setAngularLowerLimit', 'setAngularUpperLimit', 'enableAngularMotor', 'configureAngularMotor', 'disableAngularMotor'],
            conetwist: ['setLimit', 'enableMotor', 'setMaxMotorImpulse', 'setMotorTarget', 'disableMotor']
        }[this.type];
        var _this = this;
        methods && methods.map(function (fun) {
            _this[fun] = function () {
                if (arguments && arguments[0] && (typeof arguments[0] == 'object') && arguments[0].actuator) {
    ...
    Eventable.make(Physijs.Constraint);
    Physijs.Constraint.prototype.getDefinition = function () {
        var parameters = ['id', 'type', 'objecta', 'objectb', 'positiona', 'positionb',
            'axis', 'axisa', 'axisb', 'rotationa', 'rotationb','front','fronta','frontb'];
        return _.pick(this, parameters);
    };
    ...
    Physijs.Scene = function (params) {
        ...
        this._workerFile = Physijs.scripts.worker || 'physijs_worker.js';
        this._worker = new Worker(this._workerFile);
        this._worker.transferableMessage =  this._worker.postMessage || this._worker.webkitPostMessage;
        this._materials = {};
        this._objects = {};
        this._vehicles = {};
        this._constraints = {};
            ...
            if (data instanceof Float32Array) {
                ...
                if (data.cmd) {
                    ...
                    switch (data.cmd) {
                        case 'objectReady':
                            _temp = data.params;
                            if (_this._objects[ _temp ]) {
                                _this._objects[ _temp ].dispatchEvent('ready');
                                _this._objects[ _temp ].removeEvent('ready');
                            }
                            break;
                        case 'constraintReady':
                            _temp = data.params;
                            if (_this._constraints[ _temp ]) {
                                _this._constraints[ _temp ].dispatchEvent('ready');
                                _this._constraints[ _temp ].removeEvent('ready');
                            }
                            break;
                        case 'trigger':
                            _temp = data.params;
                            if (_temp.name == undefined) break;
                            var callback = _this._listeners[_temp.name];
                            if (callback === undefined) break;
                            delete _this._listeners[_temp.name];
                            callback(_temp.arg);
                            break;
                        case 'worldReady':
        ...
        var broadphases = {
            ...
            sweepprune: {
                type: 'sweepprune',
                aabbmin: Vec3(1000, 1000, 1000).negate(),
                aabbmax: Vec3(1000, 1000, 1000)
            }
        };
        params.broadphase = broadphases['dynamic'];
        this._params = params;
    ...
    Physijs.Scene.prototype.replaceWorker = function () {
        ...
        var newWorker = new Worker(this._workerFile);
        ...
        this.execute('init', this._params);
    };
    ...
    Physijs.Scene.prototype.addConstraint = function (constraint) {
    ...
    Physijs.Scene.prototype.execute = function (cmd, params) {
        this._worker.transferableMessage({ cmd: cmd, params: params });
    };
    ...
    addObjectChildren = function (parent, object) {
        ...
        for (var i = 0; i < object.children.length; i++) {
            ...
            if (child._physijs) {
                ...
                _temp_vector3_1.getPositionFromMatrix(child.matrixWorld);
            ...
            if (!child.isConnector && child.children.length) {
    ...
    Physijs.Scene.prototype.simulate = function (timeStep, maxSubSteps) {
        ...
        this.__simParams = this.__simParams || {};
        this.__simParams.timeStep = timeStep;
        this.__simParams.maxSubSteps = maxSubSteps;
        this.execute('simulate', { timeStep: timeStep, maxSubSteps: maxSubSteps });
        //this.execute('simulate', { timeStep: timeStep, maxSubSteps: maxSubSteps });
        return true;
    };

    Physijs.Scene.prototype.pause = function (pause) {
        _is_simulating = pause;
        this.execute('pause', {  pause: pause });
    };
    ...
    // Physijs.Mesh.applyCentralForce
    Physijs.Mesh.prototype.applyTorque = function (torque) {
        if (this.world) {
            this.world.execute('applyTorque', { id: this._physijs.id, x: torque.x, y: torque.y, z: torque.z });
        }
    };
    ...
    Physijs.HeightfieldMesh = function (geometry, material, mass, xdiv, ydiv) {
        ...
        for (var i = 0; i < geometry.vertices.length; i++) {
            ...
            //points[i] = geometry.vertices[i].z;
        }
